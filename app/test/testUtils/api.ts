...
export const generateEncodedUser = async (roles: string[]) => {
  const user = await UserFactory.merge({
    permissions: JSON.stringify(roles),
  }).create();

  return { token: base64.encode(`${user.email}:secret`), user };
};
...
export const createsApi = async ({
  ...
  const { token, user } = await generateEncodedUser(roles);
  ...
  return supertest(BASE_URL)
    ...
    .then(async (res) => {
      ...
      const compoundData = { ...newData, ...assertionData };
      if (addUser) {
        compoundData.user_id = user.id;
      }
      // console.log(res.body, compoundData);
      expectExceptTimestamp(res.body, compoundData);
    });
};
...
export const requiresAuth =
  (url: string, method: ApiMethod = ApiMethod.GET) =>
  async () => {
    await buildSuperInstance(url, method)
      .expect(401)
